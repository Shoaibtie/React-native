...
let dropdownIndexAttribute = {
  item: 0,
  listType: null,
};
let dropdownIndexCertificate = {
  item: 0,
  listType: null,
}

let certificateUpdate = true;
let restrictionUpdate = true;
let attributeUpdate = true;
...
class Accordian extends Component {
  ...
  update1 = (item) => {
    dropdownIndex.item = item ? 0 : -1
  }

  update2 = (item)=> {
    dropdownIndexAttribute.item = item ? 0 : -1
  }

  update3 = (item)=> {
    dropdownIndexCertificate.item = item ? 0 : -1

  }

  renderContent = (item) => {
    const { data, listType, props } = this.props;
        ...
        {listType === 'commentSection' ?
          ...
          : listType === constants.AVAILABILITY_SCREEN.SERVICE_TIMES ?
            ...
            listType === constants.AVAILABILITY_SCREEN.RESTRICTIONS ?
              <Ristrictions data={data} listType={listType} restClicked={restClicked} dropdownIndex={dropdownIndex} update1={(item) => this.update1(item) }  /> :
              listType === constants.AVAILABILITY_SCREEN.ATTRIBUTES ?
              <Attributes data={data} listType={listType} attClicked={attClicked} dropdownIndexAttribute={dropdownIndexAttribute} update2={(item) => this.update2(item)}  /> :
              listType === constants.AVAILABILITY_SCREEN.CERTIFICATIONS ?
                <Certification data={data} listType={listType} cerClicked={cerClicked} dropdownIndexCertificate={dropdownIndexCertificate} update3={(item) => this.update3(item)}  /> :
    ...
    switch(listType) {
      case constants.AVAILABILITY_SCREEN.RESTRICTIONS:
        if(dropdownIndex.item !== -1) {
          dropdownIndex.item = -1;
          dropdownIndex.listType = listType;
          this.setState({ restClicked: title, attClicked: '', cerClicked: '' });
      ...
      case constants.AVAILABILITY_SCREEN.ATTRIBUTES:
        if(dropdownIndexAttribute.item !== -1) {
          dropdownIndexAttribute.item = -1;
          dropdownIndexAttribute.listType = listType;
          
          this.setState({ attClicked: title, restClicked: '', cerClicked: ''});
      ...
      case constants.AVAILABILITY_SCREEN.CERTIFICATIONS:
        ...
        dropdownIndexCertificate.item = -1;
        dropdownIndexCertificate.listType = listType;
          
          this.setState({ cerClicked: title, restClicked: '', attClicked: '' });
